...
import { sectionsConstants } from "../constants/constant";
export const Sidebar = (props) => {
  ...
  return (
    <div className="flex flex-no-wrap h-screen">
      ...
      <div className="w-80 absolute sm:relative bg-gray-800 shadow md:h-full flex-col justify-between hidden sm:flex">
        <div className="px-3">
          <div className="h-16 w-full flex items-center">
            <div>
              ...
              <p className="text-stone-500 font-sans text-sm absolute top-8 left-9">
                SCRIPTURE MASTERY
              </p>
            </div>
          </div>
          <ul className="mt-10">
            <Sections
              activeItemClick={activeItemClick}
              itemName="liveTheNewLife"
              activeItem={activeItem}
              title="Live the New Life"
              icon={<Icon></Icon>}
            ></Sections>
            {/* <li
              className={`flex w-full justify-between cursor-pointer items-center mb-6 ${
                activeItem === "liveTheNewLife"
                  ? "text-gray-300"
            ...
            >
              <div
                ...
                onClick={() => {
                  props.series("liveTheNewLife");
                  setActiveItem("liveTheNewLife");
                }}
              >
                <Icon></Icon>
                <span className="text-sm  ml-2">Live a New life</span>
              </div>
            </li> */}
            <li
              className={`${
                activeItem === "proclaimChrist"
                  ? "text-gray-300"
                  : "text-gray-600 hover:text-gray-500"
            ...
            >
              <div
                className="flex items-center"
                onClick={() => {
                  props.series("proclaimChrist");
                  setActiveItem("proclaimChrist");
                }}
              >
                <VoiceIcon></VoiceIcon>
                <span className="text-sm  ml-2">Proclaim Christ</span>
              </div>
            </li>
            <li
              className={`${
                activeItem === "godsResources"
                  ? "text-gray-300"
                  : "text-gray-600 hover:text-gray-500"
              } flex w-full justify-between cursor-pointer items-center mb-6 `}
            >
              <div
                className="flex items-center"
                onClick={() => {
                  props.series("godsResources");
                  setActiveItem("godsResources");
                }}
              >
                <ThumbsIcon></ThumbsIcon>
                <span className="text-sm  ml-2">Rely on God's resources</span>
              </div>
            </li>
            <li
              className={`${
                activeItem === "christDisciple"
                  ? "text-gray-300"
                  : "text-gray-600 hover:text-gray-500"
              }
              flex w-full justify-between  cursor-pointer items-center mb-6`}
            >
              <div
                className="flex items-center"
                onClick={() => {
                  props.series("christDisciple");
                  setActiveItem("christDisciple");
                }}
              >
                <DiscipleIcon></DiscipleIcon>
                <span className="text-sm  ml-2">Be Christ's Disciple</span>
              </div>
            </li>
            <li
              className={`${
                activeItem === "growInChrist"
                  ? "text-gray-300"
                  : "text-gray-600 hover:text-gray-500"
              } flex w-full justify-between cursor-pointer items-center mb-6`}
            >
              <div
                className="flex items-center"
                onClick={() => {
                  props.series("growInChrist");
                  setActiveItem("growInChrist");
                }}
              >
                <PlantIcon></PlantIcon>
                <span className="text-sm  ml-2">Grow in Christlikeness</span>
              </div>
            </li>
          </ul>
      ...
      <div
        className={`h-screen w-64 z-40 absolute bg-gray-800 shadow md:h-screen flex-col justify-between sm:hidden  transition duration-150 ease-in-out ${
          sideBar ? "translate-x-0" : "translate-x-[-260px]"
        }`}
        id="mobile-nav"
      >
        <button
        ...
        <div className="px-6">
          <div className="h-16 w-full mt-4 flex items-center">
            {/* <svg
              xmlns="http://www.w3.org/2000/svg"
              width={144}
              height={30}
            ...
            <p className="text-stone-500 font-sans text-sm absolute top-11 right-12">
              SCRIPTURE MASTERY
            </p>
          </div>
          <ul className="mt-12">
      ...
      {/* container mx-auto py-10 md:w-4/5 w-11/12 px-6 */}
      <div className="container mx-auto px-6 py-6">
        {/* Remove class [ border-dashed border-2 border-gray-300 ] to remove dotted border */}
        {props.cards}
        {/* <div className="w-full h-full rounded">{props.cards}</div> */}
      </div>
    </div>
  );
};
